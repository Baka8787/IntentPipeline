...
Codex is NOT allowed to execute any Git mutation commands. The human developer owns 100% of the Git lifecycle.
...
If Gate A holds but Gate B fails, still prefer the documented process. This rule crystallizes the **B11** decision (Locomotion Mixer calibration): the tier count is tiny (Gate A weak) and the values live inside an Animancer Transition (Gate B fails), so the formula stays documented (`playback_i = threshold_i × speed_max / speed_i`) and the few asset values are configured directly. A threshold is a tunable presentation parameter, not a value Bake Data must uniquely dictate. Re-evaluate only if the count grows sharply (Strafe 2D, multiple locomotion sets).

### One-off Operation Boundary (decided 2026-08-20)
- A request such as「幫我操作」「下一步」「套用這組數值」authorizes completing that concrete operation; it does **not** authorize adding a permanent menu, button, window section, wizard, framework, or generalized workflow.
- Before touching an existing Editor tool, classify the need as **one-off asset/configuration work** or a **durable repeated workflow**. If a small, known set of assets can be configured directly once, do that (or document the SOP) and leave the tool surface unchanged.
- Reusing an existing window does not make a feature non-temporary. A feature-specific panel inside a generic tool is still permanent maintenance surface and must independently pass Gate A and Gate B.
- Build or expand a tool only when the user explicitly asks for a reusable tool/workflow **and** the gates above are satisfied after discussing scope and trade-offs. Do not infer that intent from a request for help operating Unity.
- When a one-time migration genuinely needs temporary automation, remove the temporary entry point and feature-specific code immediately after the migration. Do not leave convenience scaffolding behind.
- General usability defects in a durable tool (for example, a window that cannot scroll at its supported minimum size) may be fixed independently, but the fix must remain generic and must not smuggle the one-off feature back in.
