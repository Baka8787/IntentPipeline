...
- 🆕 **第三實例＋事件發布（M3.x-B）**：`FootstepDetector`（`Presentation/Footstep/`）是第一個 `IPresentationEventSource`——讀 Foot IK 的 **pre-IK pose 快照**判定落腳，回傳 value struct，由 `PresentationPipeline` 於順序 6.5 **末尾**合併後整體覆寫黑板 `PresentationEvents`，`AudioController` 於**下一帧**讀取播放腳步音。
  - **為什麼發布權在管線而不在偵測器**：`IPresentationController` 的「對黑板只讀不寫」契約**一個字都沒放寬**。偵測器回傳值、管線負責寫——「誰能寫黑板」與「誰負責偵測」由**型別**分開，而不是靠紀律分開。同源設計見 §4.5 的 `IArbiterSource`
  - **為什麼發布卡在所有 Controller Tick 之後**：這是「事件正確性與 Hierarchy 順序無關」的唯一依據。若在迭代中途發布，同窗口的 consumer 讀不讀得到取決於 `GetComponentsInChildren` 的回傳順序——拖動物件會變成正確性條件。代價是固定一帧延遲（約 16ms，聽不出來）
  - **廣播快照，不是可消費佇列**：consumer 只讀不清除，Audio／未來 VFX／鏡頭震動不會互相吃掉事件；不需要 sequence number、frame identity 或 acknowledgement——「恰好被每個 consumer 看到一次」是上述時序的結構結果。整體覆寫即復位，故 `ResetTransientState()` **刻意不認識它**
  - **語意：動畫落腳事件，非物理 ground contact**。刻意讀 IK 套用**前**的 pose——斜坡上 IK 會把腳修到別處，但落腳**時機**不變，而玩家聽到的聲音要對齊看到的動畫
- **定位（M2 定調；🔄 M3.x-B 擴充）**：`PresentationPipeline` 是**表現層驅動骨架**——Runner 只呼叫管線、不認識任何具體 Controller；🔄 自 M3.x-B 起**兼任表現層事件的唯一黑板發布者**（多了一件事，但仍**不做輸出仲裁**——下方「升級預留」未兌現）；後續 IK／Facial／VFX 模組沿用同一介面掛上角色階層即生效，Runner 零改動。M2 首個實例：`AudioController`（落地音；Event → Definition → Library 查表結構見 `docs/02-dev-spec.md` §3.4）
- **第二實例（✅ M3 Foot IK，M3.1 修正定調）**：`FootIKController`（Root，決策端）＋`FootIKRig`（Model，**Presentation Adapter**）——兌現 2.6 節「需要直接操作骨骼的功能掛 Model 層、與讀黑板的 Controller 分開放」的預留。兩者以**兩條獨立單向管道**橋接：`FootIKTargetData`（Controller 寫 → Rig 讀，IK 目標，**單寫單讀**）與 `FootIKPoseData`（Rig 寫 → 各 Reader 讀，動畫原始 goal 快照，🔄 M3.x-A 起**單寫多讀**）——執行期零方法呼叫，與黑板／MotionDriver 同款資料流哲學。🔄 **擁有權規則（M3.x-A）：管道的 lifetime owner ＝ 該管道的唯一 Writer**（Rig owns the *lifetime* of the pose snapshot *because it is the sole writer* of it）——這是生命週期／權威的擁有權，**不是業務擁有權**：Rig 不解讀資料、不做任何判定。此規則讓「新增第二個 Reader」不必向其他 Controller 要引用（那會違反 Controller 互不引用契約），新 Reader 向 owner 取即可、既有元件零改動。**Adapter 定義**：動畫系統邊界上的雙向轉接器，對每條管道各守單一讀寫方向、零判斷零演算法，非單純 Reader。Controller 對 Animator 零依賴（pose 一律讀快照，不採骨骼現值——反饋迴路教訓見 §5 Trade-off 與 changelog v0.18.1）。Runner 零改動（骨架的首次回收驗證）。規格見 `docs/05-foot-ik.md`（原 dev-spec §3.5，2026-07-25 分卷）
