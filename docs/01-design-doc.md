...
- **第二實例（✅ M3 Foot IK）**：`FootIKController`（Root，決策端）＋`FootIKRig`（Model，執行端）——兌現 2.6 節「需要直接操作骨骼的功能掛 Model 層、與讀黑板的 Controller 分開放」的預留。兩者以 `FootIKRuntimeData` 共享數據橋接（Controller 唯一 Writer、Rig 唯一 Reader，執行期零方法呼叫），與黑板／MotionDriver 同款資料流哲學；Runner 果然零改動（骨架的首次回收驗證）。規格見 `docs/02-dev-spec.md` §3.5
...
| **（M3 新增，已採用）** Foot IK 決策端與執行端的溝通方式 | `FootIKRuntimeData` 共享數據管道：Controller（Root）唯一 Writer、Rig（Model）唯一 Reader，組裝期 `Bind()` 注入一次，執行期零方法呼叫 | Controller 直接呼叫 Rig 方法傳目標 / IK 目標欄位併入全域黑板 `PlayerRuntimeData` / Event Bus・Callback | 直接方法呼叫讓 Root 邏輯層在執行期驅動 Model 層物件（跨 ADR-001 邊界的耦合，且時序隱含在呼叫鏈裡）；併入全域黑板則讓「表現層內部中間產物」承擔玩法契約的 Owner/Writer/Readers 治理成本、黑板變肥；共享數據＋單寫單讀與黑板／MotionDriver 哲學同款，Rig 因「值語義」（權重 0＝不生效）零決策分支，Thin Executor 可測可換 | 多一個小類別；Rig 依賴 Controller 的組裝期注入（缺 Controller 時 Rig 靜默不動——以防禦線警告補償）；未來多角色時每角色一份 RuntimeData 實例（本來就該如此） |
