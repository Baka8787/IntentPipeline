...
>
> ⚠️ **狀態：ADR-005 目前是 `Proposed`，尚未裁決。** 依 `CLAUDE.md`「同一時間只允許一個 Trial」，
> 必須等 **ADR-004 `Trial → Accepted`（工作包 P-0）** 完成後才翻牌。
> **在翻牌之前，本檔不得作為實作基線**——它是規劃產物，不是施工圖。
>
...
詳見 **ADR-005 §5**。摘要：`enum` 具語意身分（方案 B）目前領先，`int` slot index（方案 A）次之；
`ActionDefinitionSO` 參照（方案 C）違反 R3、每 Action 一個 `StateType`（方案 D）已被 ADR-004 §5.2 否決。

**選擇留到實作期**——實作會先逼出「`Damage` 該不該佔一個 slot」的答案，而那個答案會回頭決定表示法。
...
| 包 | 內容 | 前置 |
|---|---|---|
| **P-0** | **ADR-004 Acceptance**（Throw 現狀，零手感投入） | — |
...
**P-0 是硬前置**：ADR-004 未 `Accepted` 前，ADR-005 不得進入 Trial，P-A 之後全部不得開工。
