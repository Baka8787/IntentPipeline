# 專案開發更新日誌 — 歷史歸檔（Archive）

> **這是 `docs/changelog.md` 的歷史卷**：收錄 **v0.20 以前**的所有版本紀錄（v0.1 ～ v0.20）。
> 主檔只保留最近 4 個版本，理由見主檔頭部的分卷說明。
> **內容一字未改**，僅搬移；引用時的章節／版本編號與原本完全一致。
>
> 主檔（最新版本）：`docs/changelog.md`

---

## [v0.20] - ADR-003 Movement Intent Migration Stage 1：最小 seam 落地 ＋ 架構回歸測試（2026-07-25）

> ADR-003（2026-07-21 Accepted，契約定案、程式未實作）的**第一次程式落地**。本輪嚴守兩條紀律：**不改 ADR 架構**（除非發現 Blocking Issue——結論：無）、**行為等價優先**（不新增 gameplay 功能，不提前實現 AI／Network／Vehicle）。
> ⚠️ **版本號說明**：`v0.19` 保留給 Locomotion Foundation 收案（Mixer 4-tier ＋ 重烘焙，屬使用者側資產作業，待 Play 綠燈補），故本輪程式改動取 **v0.20**，兩者互不阻塞。

### 1. Stage 0：ADR-003 ↔ 現有程式對照盤點（唯讀，先盤後改）

改任何一行程式之前先做全條款盤點，結論：**ADR 五條契約（D1~D5）全部可映射到現有程式，零 Blocking Issue**。三種狀態分布——
* **已存在且相符**：`BaseState.OnUpdateMotion`（D3 的 model seam，virtual 已在）、ambient(Idle/Move) 走預設 ／ intrinsic-motion(Roll/Jump) 已 override、`AnimationFacadeBase` 維持通用 sink（D4 明言**不加** `IAnimationModel`）。
* **尚不存在**：`MovementIntent` region、`IMovementIntentSource`、`PlayerLocomotionPolicy`、`GaitProfileSO`、`MovementContext`（後者屬 Stage 3，本輪不做）。
* **存在但與 ADR 目標形態不符**：`Runner.ProcessParameters` 寫死 `input.MoveInput.magnitude → MoveSpeed`（無注入策略的縫）；B9 平滑與 `SyncAnimation` 的 `MoveSpeed` 動畫參數驅動位於通用 Runner（D4 應歸 Locomotion model）。
* **判定**：後者屬 **ADR 自己列出的 §9-L1 已知遷移項**（Stage 2），非衝突；Stage 1 只搬「意圖產生」這一段。

### 2. 落地內容（新增 5 檔／修改 5 檔）

* **黑板 seam（D1）**：`MovementIntentData{DesiredSpeedNormalized[0-1], DesiredDirection}` → `PlayerRuntimeData.MovementIntent`。**模型無關**（不含 gait 語意）、**連續型意圖**（每帧由 producer 整體覆寫，**不**參與順序 7 `ResetTransientState()`）。
* **Producer 介面（D2）**：`IMovementIntentSource`；`PlayerLocomotionPolicy`（MonoBehaviour）讀中性輸入＋`GaitProfileSO` → 寫 intent。Runner 以序列化元件欄位注入（比照既有 `inputSourceComponent` 慣例）＋同物件 `GetComponent` 補洞。
* **管線順序 2.5**：新增 Movement Intent Producer 環節，卡在 Intent Processor（2）與 Parameter Processor（3）之間。
* **中性 action 通道**：`InputData` 加 `SprintButtonHeld`／`WalkButtonHeld`（`IsPressed`，持續型）；`PlayerInputSource` 加對應 `InputAction`。**刻意不做 `[Flags] MovementModifier`**（ADR §6.3 否決：領域分類不得烤進 raw input）。
* **B9 抽成 `LocomotionSpeedSmoother`（struct，純運算）**：輸入**只有** `MovementIntentData`，`deltaTime` 由呼叫端傳入。它同時是 **Stage 2 的遷移單位**——屆時只換持有者，計算邏輯不重寫。
* **`Runner.ProcessParameters` → `DeriveMovementParameters()`**：不再讀 `InputData`，改由 intent 導出 `MoveSpeed`／`MoveDirection`／`UpperBodyWeight`。
* Editor Inspector 監視器補 Movement Intent 區與 Sprint/Walk 按鍵列（衍生值標記 `derived`）。

### 3. 行為等價是怎麼保證的（不是「應該一樣」，是「可證一樣」）

* **未指派 `GaitProfileSO` → 強度＝原始推桿量**，與 Migration 前逐位元相同；此保證有專測（`ProduceIntent_WithoutProfile_IsEquivalentToPreMigrationBehaviour`）。
* **`GaitProfileSO` 預設值一律 1＝無 gait 差異**：新建資產也不改變手感，逼設計師顯式從 Bake Data 校準（見下方 §5）。
* **`Sprint`／`Walk` InputAction 未綁定 → 恆 false**：不綁也能正常遊玩。
* **B9 的數學逐行保留**（加/減速不同時間常數、snap-to-0 清殘尾、減速期保留最後方向），只是換了輸入來源與所在型別。
* **刻意保留的兩個現況**：①producer 置於 `BlockInput` 閘門**之外**（原 `ProcessParameters` 亦然，`ArbiterPipeline` 尚無 writer）；②FSM 的 Idle/Move 門檻仍讀衍生的 `MoveSpeed`——改讀原始 intent 會讓放開輸入瞬間即切 Idle 但角色仍在滑行（動畫與位移分岔）。兩者皆列為待重估項，不在本輪偷改。

### 4. 單一真相紀律的落實（ADR §13.4，這是本輪最容易失守的地方）

Stage 1 期間 `MovementIntent` 與 legacy `MoveSpeed` 並存——**這正是 ADR-002 為 jump 物理奮戰過的「兩個真相來源」病的復發風險點**。落實方式：
* `MoveSpeed`／`MoveDirection` 在程式碼與文件上明確降級為**衍生值**（黑板 XML 註解＋dev-spec §1.1 權限表同步）。
* `LocomotionSpeedSmoother` 的輸入**只有** intent——結構上就沒有第二個真相能進來。
* 測試 `Smoother_OutputIsFullyDerivedFromIntentSequence`：同一意圖序列餵兩個獨立實例，輸出必須一致（＝無隱藏輸入）。
* A5 單一寫入者掃描：任何檔案繞過 intent 直寫 `MoveSpeed` 會讓測試變紅。

### 5. 為什麼 `GaitProfileSO` 的預設值不是 Kubold 實測值

四段速度真相（Walk 1.62／Run 3.50／Sprint 6.10 m/s）明明已經量到，卻**不**把 `0.265/0.574/1.0` 寫成程式碼預設——因為那正是 CLAUDE.md 警告的「把 Bake Data 的數字手抄進配置」：動畫資產一換，程式常數立刻過期且無人察覺（同型病：v0.15 clip 快照分岔、skinWidth/center 脫鉤）。改為**公式文件化**（`intensity_i = speed_i / speed_max` 寫進 Tooltip 與 dev-spec）＋設計師從 Bake Data 手填，與 B11「Mixer 門檻不寫自動化工具」的裁決同一條紀律。

### 6. 架構回歸測試（本輪的第二個交付物）

新增 `ArchitectureRegressionTests`（A1~A5）＋`MovementIntentTests`（A6~A8），**新增 20 條**（5＋15），`[Test]` 總數 **47 → 67**。⚠️ 順帶更正一個文件漂移：v0.18.6 起文件沿用「42 條」，但磁碟實際為 47 條（無參數化測試，逐檔 `[Test]` 計數：Audio 9／FootIK 8／MotionBake 9／MotionFeature 11／PresentationPipeline 3／StateMachineConfig 3／StateMachine 4）——推測是後續新增測試時未回頭更新計數，**以 Test Runner 實跑為準**。清單全文（含每條的「自動／人工」標註與人工項「為什麼不能自動化」）寫入 `docs/02-dev-spec.md` **§7**，依 CLAUDE.md 路由規則**不另開 ADR**。

* **A1** asmdef 依賴方向單向 ｜ **A2** `UnityEditor` 必在 `#if UNITY_EDITOR` 內 ｜ **A3** Runtime 零 LINQ
* **A4** 層級依賴禁令——含 **`Core/Movement` 禁 import `Core.StateMachine`**，這條直接把 ADR D2 的「producer context-free」變成機器可判定的約束
* **A5** 黑板單一寫入者白名單（規則表與 dev-spec §1.1 權限表刻意重複，讓「改程式忘改文件」立即現形）
* **A6** `ResetTransientState()` 不得清 `MovementIntent`（連續型 vs trigger 的分界）｜ **A7** 衍生值可重現 ｜ **A8** 行為等價 ＋ gait 解析規則

**選擇原始碼掃描而非反射**：多數不變量（誰寫黑板、哪層 import 了哪層）在編譯後已被抹平，只有原始碼層面驗得到。已知精度誠實記錄於 §7.1（只掃 Runtime、先去註解、字串內 `//` 使檢查偏寬鬆）——**刻意選「寧可漏報也不假陽性」**。

### 7. 反思（Why）

* **「架構壞了沒有症狀」是這輪做測試的真正理由**：功能壞了測試會紅，但 seam 被侵蝕（多一個黑板寫入者、producer 偷讀 state）在當下毫無症狀，直到要換 producer／加第二個 model 才爆——那時已經很貴。把可機器判定的部分固化，等於把「未來的自己會不會記得」這個變數移出方程式。
* **Stage 分割的價值**：ADR 允許 Stage 1＋2 合併，但本輪只做 Stage 1——因為 Stage 1 是**行為等價的純搬遷**（可證），Stage 2 動的是「誰擁有 dynamics」（會改變 Runner 與 model 的職責邊界）。混在一起做，一旦手感變了就分不清是搬遷錯了還是歸屬錯了。
* **誠實記錄張力，而不是假裝乾淨**：dev-spec §7.3 明列四項已知張力（B9 未歸位、trigger 意圖未分區、FSM 讀衍生值、context 軸未行使），每項都標出 ADR 依據與處理階段。**「已知的過渡」與「違規」的差別在於前者寫下來了**。

---

## [v0.18.7] - Foot IK v1 凍結：樓梯根因、A/B 收束、設計哲學轉向（2026-07-21）

> M3.5 基線 push 後的收案輪（roadmap `docs/03-animation-roadmap.md` §1.4）：使用者多輪實測後裁決**停止單點細節修補**，確立 Foot IK 設計哲學，並宣告 **Foot IK v1 凍結**（架構層健康、剩餘問題全屬「已知限制」而非缺陷）。本輪只做止血收口：程式碼回歸基線＋刪診斷段，其餘為文件同步。

### 1. 樓梯腳踝歪斜的真凶（教訓：IK 疑難先驗 collision 幾何）
* **根因確診**：長期被當成「IK 演算法缺陷」的樓梯腳踝 28° 歪斜，2026-07-20 使用者確診真凶＝**樓梯 collider 整面是一塊斜坡**（視覺 mesh 是階梯、碰撞體卻是 28°≈坡角的斜面）——raycast 命中斜面、腳掌忠實對齊斜面法線，與階梯踏面對比即「歪」，且腳掌插入斜面＝穿模。collider 修正後歪斜消失。
* **教訓**：M3.2~M3.4 三輪權重補丁（fade 族／Slope Gate／濾波）都在對著一個**環境資料錯誤**修演算法。IK 疑難的正確除錯順序：先驗 collision 幾何 → 再驗輸入資料（GetIK* 值域）→ 最後才動演算法。

### 2. GetIK* 值域疑慮否證＋診斷工具退場
* M3 交付時標註的唯一外部 API 風險——「`GetIKRotation/GetIKPosition` 在 Animancer（Playables）下值域是否正確」——經 2026-07-18 `debugLogGoals` 診斷數據排除：goal 位置與骨骼距離 ≈ 0.002m、旋轉健康、與上幀 Set 值非恆等（證明讀到的是動畫原始 goal，非我們的回寫緩衝）。**GetIK* 在 Playables 下值域正常。**
* `FootIKRig.debugLogGoals` 臨時診斷段（`#if UNITY_EDITOR`，與玩法零互動）已完成任務，本輪整段移除。

### 3. A/B 旋轉公式收束（軸對齊 vs 保留俯仰）
* v0.18.7 候選「軸對齊式」（`FromToRotation(poseUp, n)`＝主動把腳底壓平貼地）與基線「保留俯仰式」（`FromToRotation(worldUp, n)`＝保留動畫腳踝俯仰）A/B 實測**無感差**——踩地相動畫俯仰本就小（平地夾角 ~2°）。
* 依設計哲學（腳踝自由旋轉、不強制壓平）**回歸基線保留俯仰式**，軸對齊式歸檔。`ResolveFoot` 旋轉一行還原＋註解記錄結論（防未來重提）。

### 4. 設計哲學確立（自然度 > 貼合精度）
* 源自對喜好遊戲全身 IK 的觀察（大腿可高抬、小腿不受限、腳踝自由旋轉、腳底不強制整面貼地、允許少量腳尖穿模），使用者重新定義 Foot IK 目標鐵律：
  **Natural Pose > Terrain Adaptation > Perfect Foot Contact**（接受少量腳尖穿模，不接受為修穿模讓動作僵硬）。
* **明文禁令**：不再新增 Fade／Gate／大量權重修正修穿模；貼地品質改善一律走 Ground Sampling 升級（Heel/Toe 雙點、多點採樣、CapsuleCast），不走降權重。懸置的「Normal Gate 僅旋轉版」正式否決。
* **新機制檢核問句**：所有新 IK 機制必答「是否會縮小角色原本的活動空間？」——會，就必須提替代方案。
* 全文入 design-doc §4.6；哲學與 M3.2~M3.4 實驗教訓（fade＝半 IK 常態化、Slope Gate＝震盪源）的同向性見 roadmap 緣起段。

### 5. Foot IK v1 凍結宣告
* **v1 ＝ M3.5 基線＋本輪收案**。架構健康檢查全過（模組邊界／資料流／管線契約／零 GC／擴充預留，roadmap §1.2）；剩餘 6 條已知限制（L1~L6）全數文件化、各有不改架構的升級路徑（dev-spec §3.5.2）。
* 品質升級改由整體 Animation Runtime Roadmap（`docs/03`）承載，不再單點深挖：L1 跨階穿模 → 輪 7 Heel/Toe 雙點採樣；主線先進輪 2（Locomotion 資產升級＋Foot Phase 烘焙）。

### 6. 變更清單
* 程式碼：`FootIKController.ResolveFoot` 旋轉公式還原保留俯仰式（A/B 結論註解）；`FootIKRig` 刪 `debugLogGoals` 診斷段。EditMode 測試 42 條不受影響（旋轉公式非純函數測試對象、診斷段 Editor-only）。
* 文件：本 changelog v0.18.7；design-doc §4.6 補設計哲學；dev-spec §3.5 補 v1 凍結狀態＋已知限制表 L1~L6、§3.5.3 首查項標記否證、版本表補列（順修重複的 v0.18.3 列）；WORKLOG Foot IK 路線圖段改指向 `docs/03`。
* 資產（使用者側）：樓梯 collider 已修 ✅；牆壁 collider 過胖待修。

### 7. 反思（Why）
* **對錯標的修三輪的成本**：真凶是環境資料（collider 幾何），卻在 solver 上疊了三輪權重補丁——直到回歸乾淨基線＋修正 collider 才水落石出。教訓已升格為除錯順序鐵則（collision → 輸入 → 演算法）。
* **凍結是一種紀律**：架構健康、邊際收益遞減時主動封版、把後續投入導向整體 roadmap，比無止境單點深挖更符合「作品集展示品質」的目標——這正是 M3.2~M3.4 用實證換來的結論（單點＋權重補丁已到天花板）。
