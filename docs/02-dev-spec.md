...
Assets/
  Scripts/
    ...
    Presentation/        # IPresentationController, IPresentationEventSource（🆕 M3.x-B）,
                         # PresentationPipeline（表現層驅動骨架＋事件發布，順序 6.5）
...
4. **擴充＝加法**：`AIMovementSource`／`ReplaySource`／`NetworkSource` 各實作本介面，於 Inspector 換掛即切換 active producer，**Runner 不認識具體 producer**（DIP＋OCP）。🆕 P1 首個 AI producer 實證：NavMeshAgent 僅查 path／steering target，關閉 `updatePosition`／`updateRotation`；世界方向轉成既有 camera-relative `Vector2` 後寫入 `MovementIntent`，位移仍走 `LocomotionModel → MotionDriver → CharacterController.Move`。

> ✅ **第二個 producer 壓測結果（2026-08-29 P1）**：`AIMovementSource` 可直接忽略 `InputData`，現有簽名不需改動。實際失效的是 Runner 先前把「存在 `IInputSource`」當成整條管線前置條件；已改為缺席時使用 default input 並繼續。這是 Runner 入口生命週期修正，不是 producer 介面變更。
...
| `GaitProfileSO` | **只管 on-foot gait**（範圍刻意收窄，ADR-003 §6.2 的教訓：泛用 movement profile 會 overfit 到 1D speed）。換玩法控制方案＝換這顆資產 | Gait intensity 與 Mixer Threshold 都是**自由的手感參數**，目前分別以 profile 與 `Locomotion.asset` 保存；程式碼預設一律 1＝無 gait 差異，不硬編專案實測值。若 Threshold `t_i` 偏離動畫天生比例，必須以 `playback_i = t_i × speed_max / speed_i` 派生子動畫播放倍率，使錨點視覺速度與 procedural 速度一致；關閉 `SynchronizeChildren` 時，Linear Mixer 的整個混合區間亦保持循環平均速度一致。Walk 型態與 Sprint 同時成立時 **Walk 優先**（固定規則，非可調欄位）。`walkIsToggle`：hold／toggle 屬 **per-game 控制方案差異**，故住在資產而非 policy 程式碼——否則「換玩法＝換一顆資產」的承諾即破。目前只有 Walk 有實體按鍵（Sprint 規劃為 buff 驅動），故用 bool 而非 per-modifier enum，**不預造** |
...
public abstract class AnimationFacadeBase : MonoBehaviour
{
    ...
    // 通用唯讀圖查詢；無 child 概念的後端預設 false。Animancer 實作以最高權重直接子狀態為準，零配置。
...
// 🆕（M3.x-B）表現層事件來源契約：回報「本來源自己」這一帧偵測到的事件。
// **刻意回傳值**——來源因此結構上不可能寫黑板，上面那條「只讀不寫」契約不需要為事件發布放寬任何一個字。
public interface IPresentationEventSource
{
    PresentationEventData Evaluate(PlayerRuntimeData data);
}

// 集中驅動：Runner Start() 以 GetComponentsInChildren<T>() 一次性收集兩組，
// LateUpdate 順序 6.5 統一處理。陣列建構時一次配置，執行期純索引 for 迴圈＋struct 值複製，零 GC。
// 🆕（M3.x-B）兼任 PresentationEvents 的**唯一黑板寫入者**（見 §1.1 權限表）。
public class PresentationPipeline
{
    ...
    public void Tick(PlayerRuntimeData data);   // ①驅動 Controller → ②收集來源 → OR 合併 → 整體覆寫黑板
}
```

> 🔄 **定位更新（M3.x-B）**：本類別自此**多了一件事**——除了「依序驅動」，還是**表現層事件的唯一黑板發布者**。
> 這是刻意的：把發布權交給管線（而非各偵測器），才能在**不動 `IPresentationController` 契約**的前提下讓事件進黑板。
> 兩種擁有權因此明確分離：**偵測的跨帧狀態屬各來源，事件的發布與生命週期屬管線。**
> ⚠️ 它仍然**不做輸出仲裁**（多個 Controller 競爭同一個 AudioSource／骨骼）——那條預留未兌現，見 design-doc §4.6。
...
| 🆕（輪 4.1）**暫停不封鎖角色輸入** | `GamePauseController` 只切 `Time.timeScale`，未要求 `BlockInput` | **刻意的缺口**。🔬 **2026-07-27 實測拆成兩半**：<br>✅ **連續型意圖（移動）不會排隊——這是結構保證**：`MovementIntent` 每帧整體覆寫，而 B9 平滑吃 `deltaTime = 0` 推不動，`MoveSpeed` 恆為 0；放開按鍵後意圖直接歸零。此半已結案。<br>⚠️ **trigger 意圖（Jump／Roll）仍是開口**：`FullBodyStateMachine.Tick` **沒有 deltaTime 閘門**（每帧無條件 `EvaluateTransitions`），而 `JumpState.CanEnter` ＝ `JumpRequested && IsGrounded`，兩者皆與時間無關——**程式碼層面沒有任何機制阻止暫停中切入 `JumpState`**。✅ **已結案（v0.27，2026-07-27）**：`GamePauseController` 實作 `IArbiterSource`、暫停期間要求 `BlockInput`，由 Runner 的 `externalArbiterSources` 以 Inspector 引用注入（DIP）。以下保留追查全程，因為**它是「依賴不明保護」的教科書案例**。<br>🔬 **追查結論：程式碼層面缺口為真，且比原先預測更嚴重。**<br>**程式碼事實**（`JumpState`）：`CanEnter` ＝ `JumpRequested && IsGrounded`（無時間閘門）；落地判定 ＝ `_airborneTimer >= MinAirborneTimeBeforeLandingCheck && IsGrounded`，而 `_airborneTimer += deltaTime` 在暫停時**恆加 0** ⇒ `IsLanded` 永遠 false ⇒ `CanTransitionAway` 永遠 false。**推論：暫停中在觸地狀態按跳躍，FSM 會切進 `JumpState` 並卡在其中，解除暫停後才真正起跳。**<br>⚠️ **曾經寫過「意圖當帧被消耗所以不會補跳」——那是錯的**，只對連續型意圖（移動）成立；trigger 這半沒有這個保護。<br>🎯 **矛盾解開了（2026-07-27 實測）**：使用者回報「暫停中站地上按跳躍，解除後不會跳」**且**「只要站在地上，解除暫停都會聽到落地聲」。兩個觀察指向**同一個根因**——暫停時 `Move(finalMovement * 0)` ＝ `Move(Vector3.zero)`，而 Unity 的 `isGrounded` 由**上一次 Move 有沒有向下撞到東西**決定，零位移沒有向下推 ⇒ 回報 **false**。於是：暫停期間 `data.IsGrounded` 恆 false ⇒ `JumpState.CanEnter` 直接失敗（**這就是那個「不知道為何存在的保護」**）；而解除暫停後角色「重新著地」⇒ `JustLanded` 假觸發 ⇒ 落地聲。<br>⚠️ **兩者是綁在一起的**：修掉落地聲（`MotionDriver.IsTimeFrozen`）會讓 `IsGrounded` 正確地維持 true，**保護當場消失、跳躍缺口打開**。因此兩件必須同批修，這也是本列得以結案的原因。<br>📌 **本案的教訓**：文件當初寫「若沒轉移，要查出是什麼擋住的——依賴一個不知道為何存在的保護，比沒有保護更危險」。實際查出來的答案是**另一個 bug**，而且修那個 bug 會連帶打開這個缺口。若當時滿足於「實測沒事」就收案，之後任何人修好 `isGrounded` 都會無預警地讓跳躍缺口重現，且完全看不出關聯。⚠️ 注意這與上一列的 Cursor 解法**方向相反**且兩者都對：游標是「高層擁有、低層回報意圖」（App 讀角色），封鎖是「低層擁有、高層提供來源」（角色收 App 給的 source）——判準是**那個狀態的 scope 屬於誰** |
