...
> **版本規範（Versioning）**
>
> 本檔為專案版本的**唯一真實來源（SSOT）**。所有規格／設計文件的 header 一律標示「對齊專案版本 vX.Y」，不再各自維護獨立版本號。
>
> * **格式**：`v0.<minor>[.<patch>]`
> * **minor（+0.1）**：完成一個里程碑或重大架構變更（如狀態機、SRP 拆分、著地系統）。
> * **patch（+0.0.1）**：修正、除錯或小幅迭代（如 v0.4.3、v0.5.1）。
> * **凍結範圍**：`v0.5.1`（含）以前的歷史列以當時記錄為準，跨文件可能有標記落差，不追溯重編；**自 v0.6 起全專案採單一版本線**。
> * **目前版本**：**v0.7**
...
## [v0.6] - 狀態參數職責分離重構（SRP）（2026-07-11）

### 1. 變更內容
* **狀態參數資產化**：新增 `StateParamsSO`（抽象基底）與 `JumpStateParams`（`ImpulseForce` / `TakeoffDelay`），把 Jump 的物理數值從狀態邏輯抽離成可配置的 ScriptableObject。
* **拓撲與參數解耦**：`StateRule` 抽離為獨立檔案，只承載狀態拓撲；`StateMachineConfigSO` 新增 `StateParamsMapping` 與泛型安全查表 `GetStateParams<TParams>()`。
* **JumpState 資料驅動**：`Initialize` 改為查表快取物理參數，查無綁定資產時 fallback 至程式碼內建預設值（對齊 `RollState` 模式）。對應規格 `02-dev-spec.md §3.2`。

### 2. 架構設計理由（Why）
* **單一職責（SRP）**：數值（參數）、拓撲（`StateRule`）、邏輯（State 類別）三者各歸其位；日後新增有專屬參數的狀態只需新建 `XxxStateParams`，不動狀態機主體。

---

## [v0.7] - 著地資料流修正（Critical #1）與測試地基（2026-07-11）

### 1. 變更內容
* **黑板打通著地資料流**：`PlayerRuntimeData` 新增 `IsGrounded`；`MotionDriver` 於每帧物理結算後回寫真實 `CharacterController.isGrounded`。
* **進入與落地閘門**：`JumpState.CanEnter` / `RollState.CanEnter` 加上 `IsGrounded` 檢查（杜絕無限空中跳／空中翻滾）；`JumpState` 移除死碼計時器假落地，改以「起跳離地後再次讀到 `IsGrounded`」作為真實落地訊號。
* **測試地基**：導入 asmdef（`Project.Runtime` / `Project.Editor` / `Project.Tests.EditMode`）與 `StateMachineTests` EditMode 單元測試（著地閘門放行/擋下、真實落地過渡）。
* **文件治理**：導入本檔頂端「版本規範」統一版本方案，收斂原 `v0.6→v0.10` 跳號。

### 2. 架構設計理由（Why）
* **狀態機不該對物理盲眼**：著地是「能否起跳／是否落地」的物理事實，統一經黑板單一來源流入狀態機，取代與物理脫鉤的計時器。
* **可測試性即可維護性**：狀態機為純確定性邏輯；asmdef + EditMode 測試讓日後改 config／加狀態能被自動回歸擋下。
