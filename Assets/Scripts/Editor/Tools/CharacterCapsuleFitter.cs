...
{
    /// <summary>
    /// CapsuleFitter（v1.1）：對選取的 CharacterRoot 一鍵匹配 <see cref="CharacterController"/> 膠囊與 Model 對齊。
    ...
    /// <para>
    /// 量測來源：Height＝rest pose 下 Model 網格 bounds 高度；Radius＝gameplay 統一基準半徑 × Animator.humanScale
    /// （碰撞半徑屬 gameplay 參數，不隨模型胖瘦跳動；humanScale 處理不同身高縮放）。
    /// 🆕 v1.1：skinWidth 亦由本工具寫入（radius×10%）並與 center 原子綁定——skin 項若與 center 內嵌值脫鉤，
    /// 腳底偏移＝兩者差值（v1 實測教訓：事後手調 skin 必然踩中）。
    ...
    /// ⚠️ 對場景實例執行後務必 Apply 到 Prefab——只改場景物件不會回寫 Prefab 依賴（2026-07-14 教訓）。
    /// </para>
    /// <remarks>
    ...
    ///  3. stepOffset 納入自動匹配（skinWidth 已於 v1.1 納入）。
    /// </remarks>
    /// </summary>
    public static class CharacterCapsuleFitter
    {
        ...
        /// <summary>skinWidth 佔 radius 的比例（Unity 建議 ≈10%）。v1.1 起由工具寫入並與 center 原子綁定。</summary>
        private const float SkinWidthRatio = 0.1f;
        ...
        private const string MenuPath = "Tools/Project/角色 Capsule 自動對齊 (CapsuleFitter)";
        ...
        private static void FitSelected()
        {
            ...
            Undo.RecordObjects(new Object[] { model, controller }, "CapsuleFitter");
            ...
            // 緩衝間隙（PhysX contactOffset 掃掠停距）。若膠囊底直接對齊 Root 原點，角色落地後會整體
            // 懸浮 skinWidth（2026-07-14 實測：skin 0.08 → 均勻懸浮 8cm，且 0/0.03/0.08 三點斜率 1）。
            // 🆕 v1.1：skinWidth 改由本工具寫入（radius×10%，Unity 建議值）並以「同一個值」計算 center——
            // 兩者原子綁定，根絕「事後手調 skin 忘了重跑工具」造成的 center/skin 狀態脫鉤（v1 實測教訓）。
            float previousSkinWidth = controller.skinWidth;
            float skinWidth = radius * SkinWidthRatio;
            ...
            summary.AppendLine($"[CapsuleFitter] '{root.name}' 已完成匹配（可 Ctrl+Z 還原）。⚠️ 若對場景實例執行，務必 Apply 到 Prefab——場景改動不會自動回寫 Prefab 依賴：");
            ...
            summary.AppendLine($" - SkinWidth = {skinWidth:F3}（radius×10%，由工具寫入；原值 {previousSkinWidth:F3}）");
            summary.AppendLine($" - Center = (0, {height * 0.5f + skinWidth:F3}, 0) → 膠囊上抬 skinWidth，落地時 Root 原點＝腳底剛好貼地");
            summary.AppendLine(" - 註：skinWidth 與 Center 由本工具原子綁定，請勿事後單獨手調 skinWidth；如需不同 skin 比例請調整工具常數後重跑。");
